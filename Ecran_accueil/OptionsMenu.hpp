        ...
        while (data.window.pollEvent(event)) {
            if (event.type == Event::Closed) {
                ...
                if (event.key.code == Keyboard::Return) {
                    if (data.currentSelection == 5) {  // Retour au menu principal
                        inOptionsMenu = false;
                    }
                    else if (data.currentSelection < 4) {  // Modifier une touche
                        Text assignText("Please assign a touch", data.font, 24);
                        assignText.setPosition(100, 450);
                    ...
                    else if (data.currentSelection == 4) {  // Redimensionner la fenêtre
                        ...
                        while (resizing) {
                            ...
                            for (size_t i = 0; i < resolutions.size(); ++i) {
                                Text resText(to_string(resolutions[i].first) + "x" + to_string(resolutions[i].second), data.font, 24);
                                resText.setPosition(100, 150 + i * 50);
